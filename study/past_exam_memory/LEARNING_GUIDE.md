...
1. `deep_input/` で単元の芯を読む
2. `whole` を主教材にする
3. `chunk` は長い答案の修理用に使う
4. `cloze` は数値・語句・列挙名の補助確認にだけ使う

今回の再生成版では、長すぎる答案や小問が連なっているカードは、`whole` に全文を詰め込まず、答案骨子から `chunk` へ進む形に直しています。全文の暗記対象は `chunk` と `units/` 側に残しています。

再インポートするときは、先にAnki側の `生保2::過去問丸暗記` デッキを削除してください。上書きインポートだけだと、今回消した旧カードがAnki内に残ることがあります。
...
ただし、背面が `【答案骨子】` になっているカードは、全文を1枚に詰め込むと重すぎると判定したものです。そのカードは、骨子で流れを確認してから、対応する `chunk` で原文を段落ごとに再現します。
